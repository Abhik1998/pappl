...
_papplPrinterCopyAttributes(
    ...
    pappl_scanner_t *printer,		// I - Printer
    ...
    const char      *format)		// I - "document-format" value, if any
{
  ...
  pappl_sdriver_data_t *data = &printer->scan_driver_data;
					// Driver data
  ...
  {
    ipp_t *col = _papplMediaColExport(&printer->scan_driver_data, &data->media_default, 0);
					// Collection value
  ...
  if (!ra || cupsArrayFind(ra, "media-col-ready"))
  {
	...
	if (data->media_ready[i].size_name[0])
	{
          if (data->borderless && (data->bottom_top != 0 || data->left_right != 0))
	  {
	    ...
	    col = _papplMediaColExport(&printer->scan_driver_data, &media, 0);
	  ...
	  else
	  {
	    ...
	    col = _papplMediaColExport(&printer->scan_driver_data, data->media_ready + i, 0);
	    ippSetCollection(client->response, &attr, j ++, col);
	    ippDelete(col);
	  }
	}
    ...
    pappl_scanner_t *printer)		// I - Printer
{
  ...
  for (rattr = ippFirstAttribute(client->request); rattr; rattr = ippNextAttribute(client->request))
  {
    ...
    {
      for (j = 0; j < printer->scan_driver_data.num_vendor; j ++)
      {
        snprintf(defname, sizeof(defname), "%s-default", printer->scan_driver_data.vendor[j]);
        if (!strcmp(name, defname))
          break;
      }

      if (j >= printer->scan_driver_data.num_vendor)
        papplClientRespondIPPUnsupported(client, rattr);
    }
  }
  ...
  for (rattr = ippFirstAttribute(client->request); rattr; rattr = ippNextAttribute(client->request))
  {
    ...
    {
      printer->scan_driver_data.identify_default = PAPPL_IDENTIFY_ACTIONS_NONE;

      for (i = 0, count = ippGetCount(rattr); i < count; i ++)
        printer->scan_driver_data.identify_default |= _papplIdentifyActionsValue(ippGetString(rattr, i, NULL));
    }
    else if (!strcmp(name, "label-mode-configured"))
    {
      printer->scan_driver_data.mode_configured = _papplLabelModeValue(ippGetString(rattr, 0, NULL));
    }
    else if (!strcmp(name, "label-tear-offset-configured"))
    {
      printer->scan_driver_data.tear_offset_configured = ippGetInteger(rattr, 0);
    }
    else if (!strcmp(name, "media-col-default"))
    {
      _papplMediaColImport(ippGetCollection(rattr, 0), &printer->scan_driver_data.media_default);
    }
    else if (!strcmp(name, "media-col-ready"))
    {
      ...
      for (i = 0; i < count; i ++)
        _papplMediaColImport(ippGetCollection(rattr, i), printer->scan_driver_data.media_ready + i);

      for (; i < PAPPL_MAX_SOURCE; i ++)
        memset(printer->scan_driver_data.media_ready + i, 0, sizeof(pappl_media_col_t));
    }
    else if (!strcmp(name, "media-default"))
    {
      if ((pwg = pwgMediaForPWG(ippGetString(rattr, 0, NULL))) != NULL)
      {
        strlcpy(printer->scan_driver_data.media_default.size_name, pwg->pwg, sizeof(printer->scan_driver_data.media_default.size_name));
        printer->scan_driver_data.media_default.size_width  = pwg->width;
        printer->scan_driver_data.media_default.size_length = pwg->length;
      }
    }
    else if (!strcmp(name, "media-ready"))
    {
      ...
      for (i = 0; i < count; i ++)
      {
        ...
        {
          strlcpy(printer->scan_driver_data.media_ready[i].size_name, pwg->pwg, sizeof(printer->scan_driver_data.media_ready[i].size_name));
	  printer->scan_driver_data.media_ready[i].size_width  = pwg->width;
	  printer->scan_driver_data.media_ready[i].size_length = pwg->length;
	}
      }

      for (; i < PAPPL_MAX_SOURCE; i ++)
      {
        printer->scan_driver_data.media_ready[i].size_name[0] = '\0';
        printer->scan_driver_data.media_ready[i].size_width   = 0;
        printer->scan_driver_data.media_ready[i].size_length  = 0;
      }
    }
    else if (!strcmp(name, "orientation-requested-default"))
    {
      printer->scan_driver_data.orient_default = (ipp_orient_t)ippGetInteger(rattr, 0);
    }
    else if (!strcmp(name, "print-color-mode-default"))
    {
      printer->scan_driver_data.color_default = _papplColorModeValue(ippGetString(rattr, 0, NULL));
    }
    else if (!strcmp(name, "print-content-optimize-default"))
    {
      printer->scan_driver_data.content_default = _papplContentValue(ippGetString(rattr, 0, NULL));
    }
    else if (!strcmp(name, "print-darkness-default"))
    {
      printer->scan_driver_data.darkness_default = ippGetInteger(rattr, 0);
    }
    else if (!strcmp(name, "print-quality-default"))
    {
      printer->scan_driver_data.quality_default = (ipp_quality_t)ippGetInteger(rattr, 0);
    }
    else if (!strcmp(name, "print-scaling-default"))
    {
      printer->scan_driver_data.scaling_default = _papplScalingValue(ippGetString(rattr, 0, NULL));
    }
    else if (!strcmp(name, "print-speed-default"))
    {
      printer->scan_driver_data.speed_default = ippGetInteger(rattr, 0);
    }
    ...
    {
      printer->scan_driver_data.darkness_configured = ippGetInteger(rattr, 0);
    }
    ...
    else if (!strcmp(name, "printer-resolution-default"))
    {
      ...
      printer->scan_driver_data.x_default = ippGetResolution(rattr, 0, &printer->scan_driver_data.y_default, &units);
    }
    else
    {
...
{
  cups_array_t		*ra;		// Requested attributes array
  pappl_scanner_t	*printer = client->printer;
					// Printer


  if (!printer->device_in_use && !printer->processing_job && (time(NULL) - printer->status_time) > 1 && printer->scan_driver_data.status_cb)
  {
    // Update printer status...
    (printer->scan_driver_data.status_cb)(printer);
    ...
    pappl_client_t *client)		// I - Client
{
  ...
  if (client->printer->scan_driver_data.identify_cb)
  {
	...
	actions |= _papplIdentifyActionsValue(ippGetString(attr, i, NULL));
      ...
      actions = client->printer->scan_driver_data.identify_default;
    ...
    (client->printer->scan_driver_data.identify_cb)(client->printer, actions, message);
  }
